...
class TestRedactHome(unittest.TestCase):
    ...
    def test_replaces_home_prefix(self):
        ...
        self.assertEqual(_redact_home(f'{home}\\.codex\\auth.json'), '~\\.codex\\auth.json')
...
class TestDpiInfo(unittest.TestCase):
    ...
    def test_per_monitor_v2_150_percent(self):
        ...
        with patch('usage_monitor_for_codex.verbose.ctypes') as mock_ctypes:
...
class TestCredentialsStatus(unittest.TestCase):
    ...
    def test_found(self):
        ...
        with patch('usage_monitor_for_codex.verbose.Path') as mock_path, \
             patch.dict('os.environ', {}, clear=False):
            env = {k: v for k, v in __import__('os').environ.items() if k != 'CODEX_HOME'}
            with patch.dict('os.environ', env, clear=True):
                mock_home = MagicMock()
                mock_path.home.return_value = mock_home
                cred_path = mock_home / '.codex' / 'auth.json'
    ...
    def test_custom_config_dir(self):
        ...
        with patch('usage_monitor_for_codex.verbose.Path') as mock_path, \
             patch.dict('os.environ', {'CODEX_HOME': 'D:\\custom'}):
            custom_path = MagicMock()
            mock_path.return_value = custom_path
            cred_path = custom_path / 'auth.json'
            cred_path.exists.return_value = True
            result = _credentials_status()
...
class TestSetupConsole(unittest.TestCase):
    ...
    def test_attaches_parent_console_first(self):
        ...
        with patch('usage_monitor_for_codex.verbose.ctypes') as mock_ctypes, \
             ...
             patch('usage_monitor_for_codex.verbose.sys'), \
             patch('usage_monitor_for_codex.verbose.os'):
...
class TestPrintStartupDiagnostics(unittest.TestCase):
    ...
    def test_contains_all_sections(self):
        ...
        buf = io.StringIO()
        with patch('sys.stdout', buf), \
             patch('usage_monitor_for_codex.verbose.ctypes') as mock_ctypes, \
             patch('usage_monitor_for_codex.verbose.locale') as mock_locale, \
             patch('usage_monitor_for_codex.verbose.platform') as mock_platform:
